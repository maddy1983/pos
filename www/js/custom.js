...
$(function(){
    ...
    var loadItems = function() {
        $.ajax({
            ...
            success : function(data) {
                ...
                var categories_sec = '';
                categories.sort(sortByDisplayOrder);
                if(ResultCode === 0) {
                    $.each(categories, function (key,val) {
                        debugger;

                        if (val.IsActive === 1 && val.ParentCategoryID === 0) {
                            categories_sec += "<li class='categories text-center pull-left "+ val.DisplayOrder +"' id='" + val.CategoryID + "'><a>";
                            categories_sec += "<img src='" + val.ImagePath + "' class='images_list'><br/>";
                            categories_sec += "<span>" + val.CategoryName + "</span>";
                            categories_sec += "</a></li>";
    ...
    var getCategories = function(itemId) {
        ...
        var subcategories = app.Data.Categories;
        subcategories.sort(sortByDisplayOrder);
            $.each(subcategories, function (key,val) {
                if (val.ParentCategoryID.toString() === itemId) {
                    subCategories_sec += "<li class='subCategories text-center pull-left "+ val.DisplayOrder +"' id='" + val.CategoryID + "'><a>";
    ...
    var getProductList = function(itemId) {
        ...
        debugger;
            ...
            $.each(products, function (key, val) {
                if (val.CategoryID.toString() === itemId) {
                    products.sort(sortByDisplayOrder);
                    productsList_sec += "<li class='products text-center pull-left ptn_rel "+ val.DisplayOrder +"' id='" + val.ProductID + "'><a>";
                    productsList_sec += "<img src='" + val.ImagePath + "' class='images_list'><br/>";
                    productsList_sec += "<span>" + val.ProductName + "</span><br/>";
                    productsList_sec += "<p class='clearfix productUnits' id='" + val.Price + "'><span class='btn btn-primary pull-left addProductUnit productItemsBtn'>+</span>";
                    ...
                    productsList_sec += "<p class='clearfix'>$<span class='productPrice'>"+val.Price+"</span>";
                    productsList_sec += "<span class='btn btn-primary addToCart' id='product_" + val.ProductID + "'>Add to Cart</span></p>";
