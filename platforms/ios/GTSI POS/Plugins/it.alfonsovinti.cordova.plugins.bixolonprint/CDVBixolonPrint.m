...
- (void) connect
{
//    self.receiptFilePath = [[NSString alloc] init];
...
-(void)printReceipt
{
//    _pController = appDel.sharedPrinterObject;
//    printerController.delegate = self;
//    if(!appDel.isPrinterConnect )
//    {
//        [MBProgressHUD hideHUDForView:self.view animated:YES];
//        
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Printer Connection"
...
//    {
//        
//        if (![appDel.receiptFilePath isEqualToString:@""])
//        {
    ...
    self.receiptFilePath = [documentsPath stringByAppendingPathComponent:@"BillReceipt.png"]; //Add the file name
    NSLog(@"App delegate bill file path %@",self.receiptFilePath);

    NSLog(@"self.receiptFilePath:%@",self.receiptFilePath);
            int nLevel = 65;
            [printerController printBitmap:self.receiptFilePath width:BXL_WIDTH_FULL level: nLevel];
            [printerController lineFeed:2];
            if (BXL_SUCCESS == [printerController lineFeed:3])
                [printerController lineFeed:2];
//            [dataAC markOrderAsCompleted:dataAC.activeOrderId];
//            [dataAC clearProductList];
//            [self orderCompleted];
...
- (void) _printText
{
    ...
    NSDictionary* tempDict = [obj objectAtIndex:0];
    
    NSString *boxtext = [tempDict objectForKey:@"text"];

    NSLog(@"boxtext:(%@)",boxtext);
    
    NSData *data = [boxtext dataUsingEncoding:NSUTF8StringEncoding];
//    NSDictionary* jsonDic = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
    NSError* error;
    NSMutableDictionary *jsonDic = (NSMutableDictionary *) [NSJSONSerialization JSONObjectWithData:data options:0 error:&error];
    if (error) {
        NSLog(@"Error while parsing");
        
        UIAlertView* alert = [[UIAlertView alloc]initWithTitle:@"PossAPP" message:@"No items to print or error while parsing" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
        
    }else {
        NSLog(@"NO ERROR AFTER PARSING");
        
        NSLog(@"jsonDic:(%@)",jsonDic);
        
        billVC = [[BillReceiptViewController alloc] initWithNibName:@"BillReceiptViewController" bundle:nil];
        billVC.responseDictionary = jsonDic;
        [billVC setTheView];
        [self printReceipt];
    }
    
    
    //madhu commented the code
//    NSData *pngData = UIImagePNGRepresentation([self decodeBase64ToImage:base64text]);
//
//    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
//    NSString *documentsPath = [paths objectAtIndex:0]; //Get the docs directory
...
//    NSLog(@"pngDataread:%@",pngDataread);
////    NSString *path = [[NSBundle mainBundle] pathForResource:@"images/home-icon" ofType:@"png"];
//    int nLevel = 50;
//    [printerController printBitmap:filePath width:BXL_WIDTH_FULL level: nLevel];
    //end
            ...
            withError:(NSError *)error
{
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"POS App"
                                                    message:@"Couldnot connect to Printer, goto settings and pair your printer with this device."
                                                   delegate:self
                                          cancelButtonTitle:@"OK"
                                          otherButtonTitles:nil];
    [alert show];
                     ...
                     withError:(NSError *)error
{
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"POS App"
                                                    message:@"Couldnot connect to Printer, goto settings and pair your printer with this device.(Broken Connection)"
                                                   delegate:self
                                          cancelButtonTitle:@"OK"
                                          otherButtonTitles:nil];
    [alert show];
